...
function populateInputFields(rowIndex) {
    ...
    if (splitType === 0) {
        ...
        const expandableContent = row.nextElementSibling;
        if (!expandableContent || !expandableContent.classList.contains('expandable-content')) {
            console.log('Expandable content not found - will clear when expanded');
            return;
        }
        ...
        // Only clear if there are existing fields to clear
...
function clearAndPopulateContainer(container, splitData, type) {
    ...
    // Get the middle-info-top and middle-info-bottom containers
    const middleInfoTop = container.querySelector('.middle-info-top');
    const middleInfoBottom = container.querySelector('.middle-info-bottom');
    
    if (!middleInfoTop || !middleInfoBottom) {
        console.log('Middle info containers not found');
        return;
    }
...
function addAdditionalRow(container, name, ipi, share, type) {
    ...
    // Get the middle-info-bottom container
    const middleInfoBottom = container.querySelector('.middle-info-bottom');
    
    if (!middleInfoBottom) {
        console.log('Middle info bottom container not found');
        return;
    }
    ...
    // Insert into middle-info-bottom container
    middleInfoBottom.appendChild(newRow);
    console.log('Additional row inserted into middle-info-bottom container');
...
function addWriterButton(container) {
    ...
    // Insert after the middle-info-bottom container
    const middleInfoBottom = container.querySelector('.middle-info-bottom');
    if (middleInfoBottom) {
        container.insertBefore(addWriterBtn, middleInfoBottom.nextSibling);
    } else {
        // Fallback: insert before the bottom field container
        const bottomContainer = container.querySelector('.bottom-field-container');
        if (bottomContainer) {
            container.insertBefore(addWriterBtn, bottomContainer);
        } else {
            container.appendChild(addWriterBtn);
        }
    }
}
...
function addWriterRow(container) {
    ...
    console.log('New writer row added to middle-info-bottom container');
}
...
function addPublisherButton(container) {
    ...
    const middleInfoBottom = container.querySelector('.middle-info-bottom');
    if (middleInfoBottom) {
        container.insertBefore(addPublisherBtn, middleInfoBottom.nextSibling);
    } else {
        // Fallback: insert before the bottom field container
        const bottomContainer = container.querySelector('.bottom-field-container');
        if (bottomContainer) {
            container.insertBefore(addPublisherBtn, bottomContainer);
        } else {
            container.appendChild(addPublisherBtn);
        }
    }
}
